/**
 * Seeds live Supabase CMS (bypasses RLS via service role).
 * 1. Add SUPABASE_SERVICE_ROLE_KEY to .env (Dashboard → Settings → API)
 * 2. Run: node scripts/seed-cms.mjs
 */
...
async function main() {
  await upsert('site_settings', [
    ...
    { key: 'admin_primary', value: json('#46482f') },
  ], 'key')
  ...
  await admin.from('hero_stats').delete().in('label', ['successful deals', 'years experience', 'verified houses'])
  ...
  console.log('\nSeed complete. Run supabase/seed_full.sql in SQL Editor if get_public_cms_snapshot RPC is missing.')
}
